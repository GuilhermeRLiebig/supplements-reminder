...
export default function Settings({ onLogout }: { onLogout: () => void }) {
  ...
  return <section><header className="page-header"><div><p className="eyebrow">SISTEMA</p><h1>Configurações</h1></div></header>
    <div className="card settings-list"><div><span>Telegram</span><b>{data?.telegramLinked ? "Conectado ✅" : "Não conectado"}</b></div><div><span>Fuso horário</span><b>{data?.timezone || "America/Fortaleza"}</b></div><div><span>Verificação</span><b>A cada 1 min</b></div></div>
